...
import re 
import json
import glob # To find and delete old temp files
...
# --- New Imports for Image Processing with EasyOCR ---
import cv2
import easyocr
import httpx # Using httpx for robust, asynchronous HTTP requests
...
# This loads the model into memory once when the application starts for efficiency.
...
# --- New ML/CV Functions for Detail Extraction ---
def extract_chart_details_with_easyocr(image_path: str) -> List[Dict[str, Any]]:
    ...
    """
    try:
        # EasyOCR reads the image and returns a list of (bounding_box, text, confidence)
...
async def get_summary_from_extracted_data(title: str, extracted_data: List[Dict[str, Any]]) -> str:
    ...
    try:
        ...
        # Updated prompt to request a brief, single-sentence summary.
        prompt = f"""
        ...
        # Clean up response, preserving bullet points
        cleaned_summary = re.sub(r'^(here are the key findings|here is the summary|analysis:|summary):?\s*', '', summary, flags=re.IGNORECASE).strip()
        ...
        print(f"Error response from Cohere API: {e.response.status_code} - {e.response.text}")
...
async def process_image_and_get_summary(report: Report, image_data_url: str) -> Tuple[str, str]:
    """
...
def add_chart_slide(pres, report_data, image_path, summary_text):
    ...
    # Define columns for chart (left) and insights (right)
    ...
    # Add and scale the chart image to fit the left column
    ...
    # Add the insights text box on the right
    insights_box = slide.shapes.add_textbox(insights_left, insights_top, insights_width, insights_height)
    insights_frame = insights_box.text_frame
    insights_frame.word_wrap = True
    
    # Add "Key Insights" subheading
    ...
    # Add the brief summary from the AI
    p_summary = insights_frame.add_paragraph()
    p_summary.text = f"▪ {summary_text}" # Add a bullet for visual consistency
...
async def generate_ppt_endpoint(request_data: GenerationRequest):
    ...
    try:
        ...
        for report in request_data.selectedReports:
            ...
            if image_data_url:
                try:
                    image_path, summary_text = await process_image_and_get_summary(report, image_data_url)
                    temp_image_paths.append(image_path)
                    
                    add_chart_slide(pres, report, image_path, summary_text)

                except Exception as e:
                    print(f"Could not process and add slide for report {report.id}: {e}")
                    continue
